...
for y in board:
    for p in y:
        p.closest = Point(9999,9999)
        setEqual = False
        for i in coords:


            if p.closest != "Equal":
                if distanceBetween(p,i) == distanceBetween(p,p.closest):
                    setEqual = True

                elif distanceBetween(p,i) < distanceBetween(p,p.closest):
                    p.closest = i
                    setEqual = False
...
for y in board:
    for p in y:
        if p.closest != "Equal":
            p.closest.territory.append(p)

highest = Point(0,0)
for i in coords:
    i.calcArea()
    if i.area > highest.area:
        highest = i

for c in coords:
    for p in c.territory:
        if isOnEdge(p, xmax, xmin, ymax, ymin):
            c.disqualified = True
...
for p in coords:
    p.calcArea()
    if p.area > highest.area and not p.disqualified:
        highest = p
